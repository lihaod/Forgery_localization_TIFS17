...
#include "s1_spam14c.cpp"
#include "s1_minmax22c.cpp"
#include "s1_minmax24c.cpp"
#include "s1_minmax34c.cpp"
#include "s1_minmax34hvc.cpp"
#include "s1_minmax41c.cpp"
#include "s1_minmax48c.cpp"
#include "s1_minmax54c.cpp"
...
public:
	...
	s1(std::vector<float> qs, Config *config) : s(qs, config)
	{
		...
		for (int qIndex = 0; qIndex < (int)qs.size(); qIndex++)
		{
			...
			if (config->enable_c)
			{
				submodelsForQ.push_back(new s1_minmax22c(q, config));
				submodelsForQ.push_back(new s1_spam14c(q, config));
				submodelsForQ.push_back(new s1_minmax24c(q, config));
				submodelsForQ.push_back(new s1_minmax34hvc(q, config));
				submodelsForQ.push_back(new s1_minmax41c(q, config));
				submodelsForQ.push_back(new s1_minmax34c(q, config));
				submodelsForQ.push_back(new s1_minmax48c(q, config));
				submodelsForQ.push_back(new s1_minmax54c(q, config));
	...
	void ComputeImage(mat2D<int> **img, mat2D<double> * map, mat2D<int> **parity)
	{
		...
		mat2D<double> * pMap = new mat2D<double>(R[0]->rows, R[0]->cols);

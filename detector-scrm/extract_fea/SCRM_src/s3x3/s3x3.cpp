...
#include "s3x3_spam11c.cpp"
#include "s3x3_spam14c.cpp"
#include "s3x3_minmax22c.cpp"
#include "s3x3_minmax24c.cpp"
#include "s3x3_minmax41c.cpp"
...
public:
	...
	s3x3(std::vector<float> qs, Config *config) : s(qs, config)
	{
		...
		for (int qIndex=0; qIndex < (int)qs.size(); qIndex++)
		{
			...
			if (config->enable_c)
			{
				submodelsForQ.push_back(new s3x3_spam11c(q, config));
				submodelsForQ.push_back(new s3x3_spam14c(q, config));
				submodelsForQ.push_back(new s3x3_minmax24c(q, config));
				submodelsForQ.push_back(new s3x3_minmax22c(q, config));
				submodelsForQ.push_back(new s3x3_minmax41c(q, config));

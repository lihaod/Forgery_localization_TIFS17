function ann = nnmex(A,B,varargin)
...
% The following was added due to the error cause by "enrich" mode in mex
% routine.
if strcmp(varargin{1},'enrich')
    if sum(A(:)~=B(:)) > 0
        error('\nOur implementation of enrichment requires that image A = image B.\n');
    end
end
ann = nnmex_mex(A,B,varargin{:});